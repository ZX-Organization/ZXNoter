...
public abstract class AbstractComponentRenderer implements RenderInterface {
    ...
    public void transform() {
        ...
        double anchorX = switch (anchor.getHpos()) {
            case HPos.LEFT -> 0;
            case HPos.CENTER -> size.getW() / 2;
            case HPos.RIGHT -> size.getW();
        };

        double anchorY = switch (anchor.getVpos()) {
            case VPos.TOP -> 0;
            case VPos.CENTER, VPos.BASELINE -> size.getH() / 2;
            case VPos.BOTTOM -> size.getH();
        };
