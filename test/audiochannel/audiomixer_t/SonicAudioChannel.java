 ...
 * 音频句柄
 */
public abstract class SonicAudioChannel {
    /**
     * 事件监听器
     */
    ...
    public SonicAudioChannel(AudioInputStream audioData) throws IOException {
    ...
    //获取16位
    protected void read(byte[] buf, int pos, int bufSize) throws IOException {


        //输出暂存缓冲区
        byte[] outStagingBuffer = new byte[bufSize - pos];
    ...
    private final ByteArrayInputStream inputStream;//音频流
    ...
    private final AudioFormat audioFormat;//当前的音频格式
    private final Sonic sonic;//处理音频效果

    private final byte[] inBuffer;//输入缓冲区
    private final long frameLength;//帧总数
